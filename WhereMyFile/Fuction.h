...
///////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////
static void DirectoryList(const string& path, vector<string>& docs, vector<string>& file) {
...
static void showcolour(string& src, const string& key)
{
	if (src.empty() || key.empty())
		return;
	//判断属性是否相同 同为汉字同为英文
	if ((src[0] > 0&&src[0] <= 128&&key[0] > 0&&key[0] <= 128) ||
		(src[0] < 0||src[0] >= 128)&&(key[0] < 0||key[0] >= 128))
	{
		size_t pos = src.find(key);
		if (pos != string::npos) {
			string Front = src.substr(0, pos);
			string Colour = src.substr(pos, key.size());
			string End = src.substr(pos + key.size(), src.size() - pos - key.size());
			cout << Front;
			ColourPrintf(Colour.c_str());
			cout << End;
		}
	}
	else 
	{
		if ((src[0] < 0||src[0] >= 128)&&
			(key[0] > 0&&key[0] <= 128))
		{
			size_t pos = ChineseConvertPinYinAllSpell(src).find(key);
			string	colour;
			if (pos != string::npos)
			{
				size_t index = 0, front = 0, end = 0;
				//字符串不能越界
				size_t len = ChineseConvertPinYinAllSpell(src).size();
				while (index < len)
				{
					if (index == pos)
					{
					...
					//一个拼音对应汉字的长度
					size_t C_len = ChineseConvertPinYinAllSpell(src.substr(front, 2)).size();
					index +=C_len;
					front += 2;
				}
				size_t stringlen = pos + key.size()-1;
				while (index < stringlen)
				{
			   	 ...
			   	 colour = src.substr(front, end - front);
				 showcolour(src, colour);
			}
			 if ( ChineseConvertPinYinInitials(src).find(key) != string::npos)
			{
				size_t pos = ChineseConvertPinYinInitials(src).find(key);
				size_t index = 0, front = 2 * pos , end = 2 * pos ;
...
static void ShowOut(const string& key, vector<std::pair<string, string>>&  docinfos)
{
	for (auto& show : docinfos)
	{
		if (show.first.find(key) != string::npos)
		{
			showcolour(show.first, key);
